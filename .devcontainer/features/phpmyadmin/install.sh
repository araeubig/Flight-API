...
if [ "${ENABLED}" = "true" ]; then
    ...
    htpasswd -nim admin < /etc/conf.d/phpmyadmin-password > /etc/nginx/conf.extra/.htpasswd-pma
    ...
    homedir=$(getent passwd "${WEB_USER}" | cut -d: -f6)
    {
        ...
        echo "echo \"phpMyAdmin username: admin\""
        echo "echo \"phpMyAdmin password: $(cat /etc/conf.d/phpmyadmin-password || true)\""
        echo "echo"
    } >> "${homedir}/.local/share/wp-codespaces/login/050-phpmyadmin.sh"
